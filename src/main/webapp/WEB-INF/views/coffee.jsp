...
<script>
    window.onload = function () {
            ...
            if (request.readyState === 4) {
                if (request.status === 200) {
                    ...
                    products.forEach(product => {
                        ...
                        listItem.innerHTML = `
                            <div class="col-2"><img class="img-fluid" src="${img}" alt=""></div>
                            <div class="col">
                                <div class="row">${product_nm}</div>
                            </div>
                            <div class="col text-center price">${price}원</div>
                            <div class="col text-end action">
                                <button class="btn btn-small btn-outline-dark decrease" data-id="${product_id}" data-price="${price}">-</button>
                                <span id="quantity-${product_id}" class="quantity">0</span>
                                <button class="btn btn-small btn-outline-dark increase" data-id="${product_id}" data-price="${price}">+</button>
                                <button class="btn btn-small btn-outline-danger delete" data-id="${product_id}">삭제</button>
                            </div>
                        `;
                        productList.appendChild(listItem);

                        // 수량 증가 버튼 이벤트
                        document.querySelector(`.increase[data-id="${product_id}"]`).addEventListener('click', function () {
                        ...
                        // 수량 감소 버튼 이벤트
                        document.querySelector(`.decrease[data-id="${product_id}"]`).addEventListener('click', function () {
                            const price = parseInt(this.getAttribute('data-price'), 10);
                            const productId = this.getAttribute('data-id');
                        ...
                        // 삭제 버튼 이벤트
                        document.querySelector(`.delete[data-id="${product_id}"]`).addEventListener('click', function () {
                            const productId = this.getAttribute('data-id');
                            if (cart[productId]) {
        ...
        // GET 요청 전송
        request.open("GET", "/api/products", true); // 실제 상품 API 경로로 수정
        request.send();
    };
</script>
